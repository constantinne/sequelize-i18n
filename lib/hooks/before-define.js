...
module.exports = function() {
	...
	this.sequelize.beforeDefine('beforeDefine_i18n', function(model, options) {
		...
		for (var prop in model) {
			if (model.hasOwnProperty(prop)) {
				if ('i18n' in model[prop] && model[prop].i18n === true) {
					if (!pk)
						throw new Error(
							'No primary or unique key found for ' +
								options.modelName +
								' model'
						);
					schema = schema || {
		...
		if (schema) {
			// Add own primary key definition instead of sequelize default
			if (self.primary_key) {
				for (var attrname in self.primary_key) {
					schema[attrname] = self.primary_key[attrname];
				}
			}
			var name = self.constructor.getI18nName(options.modelName);
			var created_model = self.createI18nModel(
				self.constructor.getI18nName(options.modelName),
				schema,
				_options,
				options.modelName
			);
			...
			// self.setInstanceMethods(self.sequelize.Sequelize.Model.prototype, name);
